  ...
  def save( self ):
    ...
    for i in output:
      ...
      for j in range(len(i['word'])):
          ...
          verb.voice = self.voice
          verb.polarity = 'neg' if '-neg-' in i['conj'] else 'pos'

          if 'first' in i['conj']:
            verb.person = '1'
          elif 'second' in i['conj']:
            verb.person = '2'
          elif 'third' in i['conj']:
            verb.person = '3'

          if 'indicative' in i['conj']:
            verb.mood = 'ind'
          elif 'imperative' in i['conj']:
            verb.mood = 'imp'
          elif 'conditional' in i['conj']:
            verb.mood = 'cond'
          elif 'subjunctive' in i['conj']:
            verb.mood = 'sub'

          if 'imperfective_converb' in i['conj']:
            verb.aspect = 'imp'
          elif 'aorist' in i['conj'] or 'perfective_converb' in i['conj'] or 'resultative_participle' in i['conj']:
            verb.aspect = 'perf'
          elif 'future_converb-i' in i['conj'] or \
               'subjunctive' in i['conj'] or 'conditional' in i['conj']:
            verb.aspect = 'prosp'
          elif 'subject_participle' in i['conj']:
            verb.aspect = 'dur'

          if 'conditional' in i['conj'] or 'indicative' in i['conj'] or \
             'imperative' in i['conj'] or 'subjunctive' in i['conj']:
            verb.verb_form = 'fin'
          elif 'infinitive' in i['conj']:
            verb.verb_form = 'inf'
          elif 'resultative_participle' in i['conj'] or 'subject_participle' in i['conj'] or \
               'imperfective_converb' in i['conj'] or 'future_converb-i' in i['conj'] or \
               'perfective_converb' in i['conj'] or \
               'connegative_converb' in i['conj']:
            verb.verb_form = 'part'
          elif 'simultaneous_converb' in i['conj']:
            verb.verb_form = 'conv'

          if 'imperfect' in i['conj'] or 'subjunctive-future_perfect' in i['conj'] or \
             'conditional-future_perfect' in i['conj']:
            verb.tense = 'imp'
          elif 'aorist' in i['conj']:
            verb.tense = 'past'
          elif 'future' in i['conj'] or 'subjunctive-future-' in i['conj'] or \
               'conditional-future-' in i['conj']:
            verb.tense = 'pres'

          if 'passive' in i['conj']:
            verb.voice_feat = 'pass'
          elif 'caus' in i['conj']:
            verb.voice_feat = 'caus'
          else:
            verb.voice_feat = 'act'
...
class VerbParserFactory:
  ...
  def sync( cls, drop=False ):
    ...
    csv_path = os.path.join(dir_path, '../','sync/en_verbs_checked.csv')
      ...
      for row in csvreader:
        if isinstance(row, list) and len(row) >= 2:
          try:
            ...
            for v in voice:
              parser = cls.parser(row[1])()
              parser.data(row[0], row[1], {'voice': v})
              parser.save()
              n += 1
              print('Saved: ', n)
...
if __name__ == '__main__':
  ...
  parser.data('կարդալ', 'ալ', {'voice': 'middle'})
  output.append(parser.parse())
  parser = VerbParserFactory.parser('ալ')()
  parser.data('մոտենալ', 'ալ', {'voice': 'middle'})
  output.append(parser.parse())
  parser = VerbParserFactory.parser('ալ')()
  parser.data('լավանալ', 'ալ', {'voice': 'middle'})
  output.append(parser.parse())
  
  parser = VerbParserFactory.parser('ել')()
  parser.data('գրել', 'ել', {'voice': 'middle'})
  output.append(parser.parse())
  parser = VerbParserFactory.parser('ել')()
  parser.data('փախչել', 'ել', {'voice': 'middle'})
  output.append(parser.parse())
  parser = VerbParserFactory.parser('ել')()
  parser.data('ընկնել', 'ել', {'voice': 'middle'})
  output.append(parser.parse())
  parser = VerbParserFactory.parser('ել')()
  parser.data('թռչել', 'ել', {'voice': 'middle'})
  output.append(parser.parse())
  parser = VerbParserFactory.parser('ել')()
  parser.data('մտնել', 'ել', {'voice': 'middle'})
  output.append(parser.parse())
  parser = VerbParserFactory.parser('ել')()
  parser.data('հագցնել', 'ել', {'voice': 'middle'})
  output.append(parser.parse())

  file = open('parsed_verbs_middle-voice.txt', 'w+', encoding='utf-8')
