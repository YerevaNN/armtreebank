...
class NounParser:
    ...
    for i in output:
      if i[3]['word'] and i[3]['word'] != '-':
        ...
        for j in range(len(i[3]['word'])):
          ...
          noun.declension_type = i[0].split('-')[0]
          noun.declension = i[2].split('-')[0]
          noun.quantity = i[2].split('-')[1]
          noun.quantity_type = 'ներ' if i[0].split('-')[-1] == 'ներ' else 'եր'
          noun.form = '' if len(i[1]) == 2 else ( i[2].split('-')[-1] )
          noun.animate = i[4]['animate']
          noun.uncountable = i[4]['uncountable']
          noun.plural_only = i[4]['plural_only']
  ...
  def parse_html( self, base=False ):
    ...
    for d in self.declensions:
      ...
      if self.declensions.get(d)[:2] == 'LP':
        html += '</tr>'
    html += '</tbody></table>'
...
if __name__ == '__main__':
  try:
    parser = NounParserFactory.parser('ամուսնություն', '{{hy-noun-ություն}}')
    output = parser.parse()
